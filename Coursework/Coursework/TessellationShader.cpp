...
{
	initShader(L"TessellationVS.cso", L"TessellationHS.cso", L"HeightMapTessellationDS.cso", L"NoReflectPS.cso");
}
...
void TessellationShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	// Setup description for the parameters
...
void TessellationShader::setHeightMapShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, float resolution, float factor, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMap, ID3D11ShaderResourceView* depthMap2, Light* dirLight, Light* pLight1, float pSpecular1, Light* pLight2, float pSpecular2, Light* spotLight, float spotCutoff, Camera* camera, ID3D11ShaderResourceView* heightMap)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	loadDomainShader(L"HeightMapTessellationDS.cso");
	loadPixelShader(L"NoReflectPS.cso");
...
void TessellationShader::setWaveShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, float resolution, float factor, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMap, ID3D11ShaderResourceView* depthMap2, Light* dirLight, Light* pLight1, float pSpecular1, Light* pLight2, float pSpecular2, Light* spotLight, float spotCutoff, Camera* camera, float time, float frequency, float height)
{

	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	loadDomainShader(L"WaveTessellationDS.cso");
	loadPixelShader(L"ReflectPS.cso");
	...
	deviceContext->DSSetConstantBuffers(2, 1, &waveBuffer);
