...
float4 main(InputType input) : SV_TARGET
{
    ...
    // Shadows applied to appropriate lights
    textureColour = texture0.Sample(sampler0, input.tex);
    lightColour = calculateDirectionalLighting(-dirLightDirection, input.normal, dirDiffuseColour) * shadow1Colour;
    lightColour += calculatePointLighting(p1DiffuseColour, p1Position, input.worldPosition, cameraPosition, input.normal, 9.0f);
    lightColour += calculatePointLighting(p2DiffuseColour, p2Position, input.worldPosition, cameraPosition, input.normal, 9.0f);
