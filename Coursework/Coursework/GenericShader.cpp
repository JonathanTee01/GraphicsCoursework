...
GenericShader::GenericShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"VertexShader.cso", L"ReflectPS.cso");
}
...
void GenericShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void GenericShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* depthMap, ID3D11ShaderResourceView* depthMap2, Light* dirLight, Light* pLight1, float pSpecular1, Light* pLight2, float pSpecular2, Light* spotLight, float spotCutoff, Camera* camera)
{
	HRESULT result;
